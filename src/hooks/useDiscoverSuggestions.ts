...
export const useDiscoverSuggestions = () => {
  ...
  const buildDiscoverPrompt = (
    ...
    if (filters.setting) {
      lines.push(`- Setting: ${filters.setting}`);
    }

    if (filters.budget) {
      lines.push(`- Budget for two people: ${filters.budget}`);
    }
    ...
    if (filters.time) {
      lines.push(`- Time available: ${filters.time}`);
    }

    if (filters.energy) {
      lines.push(`- Energy level: ${filters.energy}`);
    }
    ...
    if (filters.weather) {
      lines.push(`- Weather: ${filters.weather}`);
    }
